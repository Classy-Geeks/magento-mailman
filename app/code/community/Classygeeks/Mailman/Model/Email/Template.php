...
class Classygeeks_Mailman_Model_Email_Template extends Mage_Core_Model_Email_Template
{
	/**
	 * Variables
	 */
    ...
    {
	    // Set
	    $this->getMail()->setFromName($sEmail);
	...
	/**
	 * Send
	 * @param array|string $sEmail
	 * @param null $sName
	 * @param array $aVariables
	 * @return bool
	 */
	public function send($sEmail, $sName = null, array $aVariables = array())
	{
		// Enabled?
		if (!Mage::getStoreConfig('mailman/settings/enabled')) {
			return parent::send($sEmail, $sName, $aVariables);
		}
		...
		try {
			...
			// Prepare emails and names
			$aEmails = array_values((array) $sEmail);
	        $aNames = (is_array($sName) ? $sName : array($sName));
	        $aNames = array_values($aNames);
	        foreach ($aEmails as $sKey => $sEmail) {

		        // -- Set
	            if (!isset($aNames[$sKey])) {
	                $aNames[$sKey] = substr($sEmail, 0, strpos($sEmail, '@'));
	            }

		        // -- Add to
		        $this->addTo($sEmail, $aNames[$sKey]);
	        }

			// Set variables for the template
	        $aVariables['email'] = reset($aEmails);
	        $aVariables['name'] = reset($aNames);
		    ...
		    $aFrom = array(
			    'email' =>  $sFromEmail,
			    'name'  =>  (empty($sFromName) ? '' : $sFromName)
		    );
		    ...
		    // Set subject
		    $oMessage->setSubject($this->getProcessedTemplateSubject($aVariables));
		    ...
		    // -- Compress
			$sBody = $oFilterCompress->filter($this->getProcessedTemplate($aVariables, true));
		    // -- Make file hash
	        ...
	        // Log
		    Mage::logException($e);
		    Mage::log("Error with Mail Man Email Template Send: {$e->getMessage()}", Zend_Log::CRIT, 'mailman.log');
